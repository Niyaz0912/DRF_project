...
class SectionTestCase(APITestCase):
    ...
    def test_section_delete(self):
        response = self.client.post('/section/3/delete/')
        print(response)
    ...
    def test_section_update(self):
        ...
        response = self.client.post('/section/6/update/', data=data)
        print(response.json())
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()['title'], 'test_section_put')
        self.assertEqual(response.json()['description'], 'test_section_put')
